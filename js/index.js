...
function doneTask(event){
 ...
 function pinnedTask(event){
     ...
     if (index === -1) {
        return alert('Die Aufgabe hat nicht gefunden!')
     }

     if (!arrayTasksLS[index].pinned && arrayTasksLS[index].done) {
         return alert('Um die Aufgabe zu sichern,entfernen zuerst die Markierung über die Ausführung!,to fix task.first remove thr mark of completion');
 ...
 function saveEditedTask(task){
    ...
    if (editedTaskIndex !== -1) {
        arrayTasksLS[editedTaskIndex].tsak = task;
        setTasksLocalStorage(arrayTasksLS);
        updateListTasks();
    }else{
        alert('notwendige Aufgabe  hat nicht gefunden!')
    }
    resetSendForm();
 }
